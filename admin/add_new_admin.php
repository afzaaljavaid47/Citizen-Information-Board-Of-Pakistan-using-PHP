        ...
        <ul class="navbar-nav mb-md-3">
          ...
          <li class="nav-item">
           ...
           <li class="nav-item active">
            <a class="nav-link active" href="add_new_admin.php">
...
<h1 style="text-align: center;font-weight: bold;padding-top: 20px;color: white;">Add a New Admin</h1>
<p style="text-align: center;">Only admins with a secret password can add a new admin</p><hr width="80%" style="background-color: white;">
            <div class="row">
  ...
  <div class="form-group">
    <input type="submit" style="font-size: 20px;letter-spacing: 2px;" name="submit_signup" id="submit_btn" value="Add New Admin" class="btn btn-primary btn-block">
</div>
  </form>
  </div>
</div>
...
if($pas==$confirm)
{
$sqll="SELECT * FROM `admins` WHERE `secret_passowrd`='$sec' AND `id`='$un_id'";
$runn=mysqli_query($con,$sqll);
$row=mysqli_fetch_row($runn);
if($row>=1)
{

$sql_query="SELECT * FROM `admins` WHERE `user_name`='$un'";
$run_query=mysqli_query($con,$sql_query);
$rows=mysqli_fetch_row($run_query);
if($rows<1)
{
...
else
{
	  ?>
      <script type="text/javascript">
        alert("User Name already Exists ! Try with other User Name.");
      </script>
      <?php
}
}
...
else
{
	?>
	<script type="text/javascript">
		alert("Password and Secret Password Should be Same ! ");
	</script>
	<?php
}
}
